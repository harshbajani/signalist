    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {news.map((article) => (
          <Card
            key={article.id}
            className="bg-[#1a1a1a] border border-[#2a2a2a] hover:bg-[#1f1f1f] transition-colors duration-200"
          >
            <div className="p-4">
              {/* Stock badge at the top */}
              {article.related && (
                <div className="mb-3">
                  {getStockBadgeFromRelated(article.related)
                    .slice(0, 1)
                    .map((symbol, index) => (
                      <span
                        key={index}
                        className="inline-block text-teal-400 bg-[#10B981]/20 px-2 py-1 rounded text-xs font-mono font-bold"
                      >
                        {symbol.trim()}
                      </span>
                    ))}
              ...
              {/* Title */}
              <h3 className="text-white font-semibold text-sm line-clamp-2 leading-tight mb-3 min-h-2">
                {article.headline}
              </h3>
              ...
              {/* Summary */}
              <p className="text-xs text-gray-400 line-clamp-3 mb-4 min-h-4 leading-relaxed">
                {article.summary}
              </p>
              ...
              <div className="flex items-center justify-between">
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-1 text-xs text-yellow-400 hover:text-yellow-500 transition-colors"
                >
                  Read More →
                </Link>
              </div>
