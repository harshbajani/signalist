...
export const searchStocksWithWatchlistStatus = async (
  query?: string,
  userEmail?: string
  ...
  try {
    ...
    if (!userEmail || stocks.length === 0) {
      return stocks;
    }

    const { checkWatchlistStatus } = await import(
      '@/lib/actions/watchlist.actions'
    );
    const symbols = stocks.map((stock) => stock.symbol);
    const watchlistStatus = await checkWatchlistStatus(userEmail, symbols);
